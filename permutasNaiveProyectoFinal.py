...
cadena_grande = (
    ...
    "hola universo, hola ATGTTTGTTT, hola adios solar, hola estrella, hola planeta. "
    ...
    "Este texto es bye largo y puede repetirse varias veces para simular un corpus real. "
) 
...
def agregarDatoTabla(dato):
    niveles.append(tabla.insert("","end",text=dato,image=img_adn));
...
def generarPermutas():
    ...
    botonBusqueda.config(state="active", bg="yellow", cursor="hand2");
...
def realizarBusqueda():
    ...
    for listaPermutas in listaCombinacionesPatrones:
        ...
        for permuta in listaPermutas:
            ...
            combinacion.append(busqueda.divide_and_conquer_search(bit_grande,bit_patron,0));
        listasCombinacionesNaive.append(list(combinacion));
    agregarDatosNaive();
    botonBusqueda.config(state="disabled", bg="gray", cursor="X_cursor");
...
root = tk.Tk()
img_adn = tk.PhotoImage(file="C:/Users/migue/Desktop/proyecto/Proyecto_final_Analisis_Algoritmos/adn.png").subsample(20,20);
...
etiquetaBusqueda =tk.Label(entradaPatronesFrame,text="Generar Busqueda");
botonBusqueda= tk.Button(entradaPatronesFrame,text="Busqueda",width=20,height=1,bg="gray",state="disabled",cursor="X_cursor",command=realizarBusqueda);
etiquetaBusqueda.grid(row=2,column=1,sticky="w");
botonBusqueda.grid(row=2,column=2,pady=5,padx=5);
mensajePatronFrame.pack(pady=5);
tabla=ttk.Treeview(mostrarPatronesFrame, columns=("Patrones","No. Encontrados","Posiciones"), show="tree headings");
